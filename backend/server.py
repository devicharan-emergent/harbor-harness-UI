...
# This replaces the old "any @emergent* address" gate: now only explicitly
# added operators can log in and run evals. Roles: "admin" (can manage the
# list) | "member" (can use the app). Seeded admins come from SEED_ADMIN_EMAILS.
    ...
    if not await _is_allowed_email(email):
        ...
        if email:
            try:
                ...
                pass
        # Message distinguishes the two failure modes but always mentions
        # "@emergent" so the wording stays accurate for domain rejections.
...
async def admin_add_user(
    ...
    if not _email_domain_ok(email):
        raise HTTPException(
            ...
            detail={
                "error": "invalid_email",
                "message": "Only @emergent* email addresses can be added.",
            },
        )
    now = datetime.now(timezone.utc)
    await db.allowlist.update_one(
...
async def get_credits(_user: Dict[str, Any] = Depends(require_admin)):
    ...
    Source of truth lives in agent-service Postgres, which this UI backend
    can't reach directly — so we proxy the harness's GET /api/v1/credits
    (which reads it). Degrades gracefully to {"available": false, ...} so the
    dashboard can render "unavailable" instead of erroring while that harness
    endpoint / its DB wiring is being rolled out. Admin-gated: the balance is
    operational info, not for every member.
    """
    try:
